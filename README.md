...
## Plumber campaign fixture (lane B2)

`requirements.txt` is pinned to versions with known, already-fixed HIGH/CRITICAL
advisories. The expected behaviour is:

1. the secure pipeline's Trivy gate fails the run,
2. `GET /api/github/pipeline-remediation` previews a set of dependency bumps,
3. `POST` to the same route opens a fix PR,
4. the re-run passes.

If step 2 reports "not auto-fixable", the deterministic remediation path is the
thing under test, not this repo.
